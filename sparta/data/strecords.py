...
from sparta.common import Tile, convert_map, tile_bounds
...
def load_samples(tile, train, val):
    ...
    datas = {
        "image_t": zarr.open(f'./predictions/spatial_h{tile.h:02}v{tile.v:02}_1984_2021.zarr')[list(range(1984, 2021)).index(2001): list(range(1984, 2021)).index(2019) + 1, :, :, :],
...
if __name__ == '__main__':
    ...
    tile = Tile(h, v)
    mkdirs(f"{outdir}/sparta/train/h{h:02}v{v:02}")
    mkdirs(f"{outdir}/sparta/val/h{h:02}v{v:02}")
    ...
    if part == 'train':
        ...
        _ = pool_map(partial(create_records, data=train_samples, path=f"{outdir}/sparta/train/h{h:02}v{v:02}", nsamples=tnsamples), range(tnrecords))
    
    elif part == 'val':
        print('val')
        val_samples = load_samples(tile, False, True)
        vnsamples, vnrecords = samples_records(val_samples, n=100)
        print(vnsamples, vnrecords)
        _ = pool_map(partial(create_records, data=val_samples, path=f"{outdir}/sparta/val/h{h:02}v{v:02}", nsamples=vnsamples), range(vnrecords))