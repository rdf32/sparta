...
from sparta.common import read_image_offsets, get_proj_rio, average_ensemble
...
def create_zarr(outpaths: dict, profile: dict): #bounds, projection, gsd, min_year, max_year, models
    ...
    slzarray = zarr.open(outpaths[f"sparta_{profile['model']}"],
                mode='w',
...
def predict(chunk: Block, inpaths: dict, outpaths: dict, profile: dict) -> dict:
    ...
    tprobs, ttime = run_time(predict_transformer)(profile['models']['tformer']['all'][0], 
                                                  spatialprobs[:, :, profile['rowoff']:mchunk+profile['rowoff'], profile['coloff']:mchunk+profile['coloff']], mchunk, profile)
    output = zarr.open(outpaths[f"sparta_{profile['model']}"], 'a')
...
if __name__ =='__main__':
    ...
    outpaths = {output: f"{config['pred_output']}/predictions/{output}_h{h:02}v{v:02}_{config['min_year']}_{config['max_year']}.zarr" 
        for output in [f"sparta_{config['model']}"]}
    
    config['models']['unets'] = [tf.keras.models.load_model(path, compile=False) for path in config['models']['unets']]
    config['models']['tformer']['all'] = [tf.keras.models.load_model(path, custom_objects={"PositionalEmbedding": PositionalEmbedding(embed_dim=18)}, compile=False) for path in config['models']['tformer']['all']]
